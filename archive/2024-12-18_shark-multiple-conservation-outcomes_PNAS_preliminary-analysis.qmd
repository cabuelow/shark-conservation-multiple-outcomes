...
#load('outputs/models/global_zinb_mpacompliance.rda')
...
The negative effect of MPA presence is counter to the positive causal effect estimated in Klinard et al. (*in prep*). We are in the process of solving this issue.
...
base_preds_zinb <- dat_compliance |> 
  add_predicted_draws(fit_zinb_int_mpacompliance) |> 
...
fit_prior_lognormal_int <- brm(ingestion_C_g_day ~ mpa_present + mpa_age + mpa_area + mpa_compliance + gear_limits + species_limits + catch_limits + effort_limits + size_limits + temporal_limits + HDI_2015 + I(HDI_2015^2) + gov_effect_2016 + population_2016 + shark_protection_status + Grav_Total + shark_sanctuary + shark_protection_status:Grav_Total + (1|region_id/location_id/reef_id/set_id),
                                        prior = c(prior(normal(0, 2), class = b)), # leaving intercept and sd as default priors
                                        iter = 2000, warmup = 1000, cores = 4, chains = 4, thin = 1,
...
fit_lognormal_int <- brm(ingestion_C_g_day ~ gear_limits + species_limits + catch_limits + effort_limits + size_limits + temporal_limits + HDI_2015 + I(HDI_2015^2) + gov_effect_2016 + population_2016 + shark_protection_status + Grav_Total + shark_sanctuary + shark_protection_status:Grav_Total + (1|region_id/location_id/reef_id/set_id),
                                  prior = c(prior(normal(0, 2), class = b)), # leaving intercept and sd as default priors
                                  iter = 2000, warmup = 1000, cores = 4, chains = 4, thin = 1,
                                  data = dat_flux, family = lognormal, 
                                  control = list(max_treedepth = 15, adapt_delta = 0.99))

fit_prior_guassian_int <- brm(log(ingestion_C_g_day) ~ mpa_present + mpa_age + mpa_area + mpa_compliance + gear_limits + species_limits + catch_limits + effort_limits + size_limits + temporal_limits + HDI_2015 + I(HDI_2015^2) + gov_effect_2016 + population_2016 + shark_protection_status + Grav_Total + shark_sanctuary + shark_protection_status:Grav_Total + (1|region_id/location_id/reef_id/set_id),
                                        prior = c(prior(normal(0, 2), class = b)), # leaving intercept and sd as default priors
                                        iter = 2000, warmup = 1000, cores = 4, chains = 4, thin = 1,
                                        data = dat_flux, #family = lognormal, 
                                        control = list(max_treedepth = 15, adapt_delta = 0.99),
...
#| error: false
#| warning: false
pp_check(fit_prior_zinb_int_mpacompliance) + xlim(c(0,30))
```
...
#| error: false
#| warning: false
plot(fit_zinb_int_mpacompliance)
```
...
#| error: false
#| warning: false
summary(fit_zinb_int_mpacompliance)
```
...
qresids_int <- createDHARMa(
  simulatedResponse = t(posterior_predict(fit_zinb_int_mpacompliance)),
  observedResponse = fit_zinb_int_mpacompliance$data$shark_maxn,
  fittedPredictedResponse = apply(t(posterior_epred(fit_zinb_int_mpacompliance)), 1, mean),
  integerResponse = TRUE)
save(#qresids, 
...
#| error: false
#| warning: false
dat_compliance$set_lat2 <- dat_compliance$set_lat
dat_compliance$set_long2 <- dat_compliance$set_long
while(nrow(dat_compliance[which(duplicated(select(dat_compliance, set_lat2, set_long2))),])>0){
  dat_compliance$duplicated <- duplicated(select(dat_compliance, set_lat2, set_long2))
  dat_compliance <- dat_compliance |> 
...
testSpatialAutocorrelation(qresids_int, dat_compliance$set_long2, dat_compliance$set_lat2)
```
...
#| error: false
#| warning: false
pp_check(fit_zinb_int_mpacompliance)
```
